package com.latico.commons.office.xls.poi;
...
public class Excel {
	 ...
	 */
	public Excel(String xlsFilePath) throws IOException {
	 ...
	 */
	public Excel(File xlsFile) throws IOException {
	 ...
	 */
	private boolean init(String xlsFilePath) throws IOException {
		boolean isOk = false;
		if(xlsFilePath == null) {
			return isOk;
		}
	 ...
	 * @return true: 初始化成功; false:初始化失败
	 */
	private boolean init(File xlsFile) throws IOException {
		boolean isOk = false;;
	...
	private String _getDefaultSheetName() {
		return StringUtils.join("Sheet", (workbook.getNumberOfSheets() + 1));
	}
	...
	public Sheet createSheet(String name) {
		Sheet sheet = Sheet.NULL;
		try {
			org.apache.poi.ss.usermodel.Sheet poiSheet = StringUtils.isEmpty(name) ?
	...
	public int createPagingSheets(List<String> header, List<List<Object>> datas,
		...
		int page = 0;
		if(init == false || CollectionUtils.isEmpty(datas)) {
			return page;
		}

		if(StringUtils.isEmpty(sheetNamePrefix)) { sheetNamePrefix = "PAGING-SHEET-"; }
		if(pageRowLimit > MAX_ROW()) { pageRowLimit = MAX_ROW(); }
		final int ROW_OFFSET = (CollectionUtils.isEmpty(header) ? 0 : 1);

		int pageRowCnt = 0;
		Sheet sheet = null;
		for(int size = datas.size(), row = 0; row < size; row++) {
			if(pageRowCnt == 0) {	// 新建sheet页
				String sheetName = StringUtils.join(sheetNamePrefix, ++page);
	 ...
	 * @return true:成功; false失败
	 */
	public boolean saveAs(String filePath) throws IOException {
		boolean isOk = false;
