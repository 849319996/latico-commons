...
public class NumConvertUtils extends org.apache.commons.lang3.Conversion {
	 ...
	 * @return 可视的十六进制的字符串
	 */
	public static String toHexUpperCase(byte[] bytes) {
		if(bytes == null) {
			return "";
		}
	 ...
	 * @return 可视的十六进制的字符串
	 */
	public static String toHexLowerCase(byte[] bytes) {
		if(bytes == null) {
			return "";
		}
	 ...
	 * @return 可视的十六进制的字符串
	 */
	public static String toHexUpperCase(byte[] bytes, int offset, int len) {
		if(bytes == null || bytes.length <= offset || len < 0) {
			return "";
		}

		return toHex(bytes, offset, len).toUpperCase();
	}
	 ...
	 * @return 可视的十六进制的字符串
	 */
	public static String toHexLowerCase(byte[] bytes, int offset, int len) {
		if(bytes == null || bytes.length <= offset || len < 0) {
			return "";
		}

		return toHex(bytes, offset, len).toLowerCase();
	}
	...
	public static long binToDec(String bin) {
		long dec = 0L;
		if(bin != null) {
			char[] binarys = StringUtils.reverse(bin.trim()).toCharArray();
			dec += ('0' == binarys[0] ? 0 : 1);
	...
	public static String decToBin(String dec) {
		return Long.toBinaryString(NumberUtils.toLong(dec));
	}
	...
	public static String decToOct(String dec) {
		return Long.toOctalString(NumberUtils.toLong(dec));
	}
	...
	public static String decToHex(String dec) {
		return Long.toHexString(NumberUtils.toLong(dec)).toUpperCase();
	}
