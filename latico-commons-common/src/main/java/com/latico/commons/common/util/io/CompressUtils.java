...
public class CompressUtils {
	...
	/** 默认压缩编码 */
	public final static String DEFAULT_ENCODE = CharsetType.UTF8;
	...
	public static String compress(String filePath, FileType fileType) {
		...
		String snkPath = StringUtils.join(filePath, fileType.EXT);
	...
	public static boolean compress(String srcPath, String snkPath, FileType fileType) {
		if(StringUtils.isEmpty(srcPath) || StringUtils.isEmpty(snkPath)){
	 ...
	 * @return true:压缩成功; false:压缩失败
	 */
	public static boolean toZip(String srcPath, String zipPath) throws IOException {
		boolean isOk = true;
	 ...
	 * @return true:压缩成功; false:压缩失败
	 */
	public static boolean toZip(String[] srcPaths, String zipPath) throws IOException {
		if(srcPaths == null || srcPaths.length <= 0 || 
				StringUtils.isEmpty(zipPath)) {
			return false;
		}
		
		if(!FileType.ZIP.EXT.equals(FileUtils.getExtensionName(zipPath))) {
			return false;
		}
	...
	private static void addFileToZip(ZipArchiveOutputStream zos, 
		...
		String entryName = StringUtils.join(baseDir, file.getName());
	 ...
	 * @return true:压缩成功; false:压缩失败
	 */
	public static boolean toTar(String srcPath, String tarPath) throws IOException {
		boolean isOk = true;
	 ...
	 * @return true:压缩成功; false:压缩失败
	 */
	public static boolean toTar(String[] srcPaths, String tarPath) throws IOException {
		if(srcPaths == null || srcPaths.length <= 0 || 
				StringUtils.isEmpty(tarPath)) {
			return false;
		}
		
		if(!FileType.TAR.EXT.equals(FileUtils.getExtensionName(tarPath))) {
			return false;
		}
	...
	private static void addFileToTar(TarArchiveOutputStream tos, 
		...
		String entryName = StringUtils.join(base, file.getName());
		...
		if (file.isFile()) {
			...
			File[] cFiles = file.listFiles();
			for (File cFile : cFiles) {
				addFileToTar(tos, cFile, StringUtils.join(entryName, "/"));
			}
		}
	}
	...
	public static boolean extract(String srcPath, String snkDir) {
		if(StringUtils.isEmpty(srcPath) || StringUtils.isEmpty(snkDir)){
	 ...
	 * @return true:解压成功; false:解压失败
	 */
	public static boolean unZip(String zipPath) throws IOException {
	 ...
	 * @return true:解压成功; false:解压失败
	 */
	public static boolean unZip(String zipPath, String snkDir) throws IOException {
		boolean isOk = true;
	 ...
	 * @return true:解压成功; false:解压失败
	 */
	public static boolean unTar(String tarPath) throws IOException {
	 ...
	 * @return true:解压成功; false:解压失败
	 */
	public static boolean unTar(String tarPath, String snkDir) throws IOException {
		boolean isOk = true;
		FileUtils.createDir(snkDir);
	 ...
	 * @return true:解压成功; false:解压失败
	 */
	public static boolean unGZip(String gzipPath) throws IOException {
	 ...
	 * @return true:解压成功; false:解压失败
	 */
	public static boolean unGZip(String gzipPath, String snkDir) throws IOException {
		boolean isOk = true;
		FileUtils.createDir(snkDir);
	 ...
	 * @return true:解压成功; false:解压失败
	 */
	public static boolean unBZ2(String bzPath) throws IOException {
	 ...
	 * @return true:解压成功; false:解压失败
	 */
	public static boolean unBZ2(String bzPath, String snkDir) throws IOException {
		boolean isOk = true;
		FileUtils.createDir(snkDir);
	...
	public static String toGZipString(final String str, final String encode) {
		...
		if (str != null && str.length() > 0) {
			try {
				...
				gos.close();
				bos.close();
				hex = NumConvertUtils.toHexUpperCase(bos.toByteArray());
				
			} catch (Exception e) {
				LOG.error("压缩字符串失败: [{}]", StringUtils.abbreviate(str), e);
			}
		}
		return hex;
	}
	...
	public static String unGZipString(String hex, final String encode) {
		...
		byte[] bytes = NumConvertUtils.toBytes(hex);
		if(bytes != null) {
			try {
				ByteArrayOutputStream bos = new ByteArrayOutputStream();
				...
				LOG.error("解压字符串失败: [{}]", StringUtils.abbreviate(hex), e);
			}
		}
		return str;
	}
