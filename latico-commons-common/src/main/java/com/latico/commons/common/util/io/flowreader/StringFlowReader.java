 ...
 * 
 * 使用示例:
 * 	StringFlowReader sfr = new StringFlowReader(String, CharsetType.UTF8);
...
public class StringFlowReader {
	...
	private static final Logger log =
			LoggerFactory.getLogger(StringFlowReader.class);
	
	/** 所读入字符串的默认编码 */
	public final static String DEFAULT_CHARSET = CharsetType.ISO;
	
	/**
	 * 所读入字符串的默认[行终止符].
	 * (默认为换行符, 但存在无换行符的流式文件需要另外指定)
	 */
	public final static char DEFAULT_LINE_END = CharUtils.CR;
	...
	public String readLine(char lineEnd) {
		...
		try {
			while(true) {
				...
				if(c == lineEnd || c == CharUtils.CR || c == CharUtils.LF) {	// 已到行尾
					break;
				}
			}
		} catch (IOException e) {
