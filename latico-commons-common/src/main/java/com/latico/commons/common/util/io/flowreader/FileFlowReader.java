package com.latico.commons.common.util.io.flowreader;
 ...
 * 
 * 使用示例:
 * 	FileFlowReader ffr = new FileFlowReader(FILE_PATH, CharsetType.UTF8);
...
public class FileFlowReader {
	...
	/** LOG 日志工具 */
	private static final Logger LOG = LoggerFactory.getLogger(FileFlowReader.class);
	
	/** 所读入文件的默认编码 */
	public final static String DEFAULT_ENCODE = CharsetType.ISO;
	
	/**
	 * 所读入文件的默认[行终止符].
	 * (默认为换行符, 但存在无换行符的流式文件需要另外指定)
	 */
	public final static char DEFAULT_LINE_END = CharUtils.LF;
	...
	public String readLine(char lineEnd) {
		...
		try {
			while(true) {
				...
				// 已到行尾
				if(c == lineEnd || c == CharUtils.LF || c == CharUtils.CR) {
					break;
				}
			}
		} catch (IOException e) {
