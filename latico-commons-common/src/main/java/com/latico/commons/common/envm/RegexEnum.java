...
public enum RegexEnum {
	...
	DATE(StringUtils.join("(", YEAR, "-", MONTH, "-", DAY, ")"), "日期"),
	...
	TIME(StringUtils.join("(", HOUR, ":", MINUTE, ":", SECOND, "(\\.", MILLIS, ")?)"), "时间"),
	
	DATE_TIME(StringUtils.join("(", DATE, " ", TIME, ")"), "日期时间"),
	
	MAC("([a-fA-F\\d]{2}([:\\s-]?[a-fA-F\\d]{2}){5})", "MAC: 物理地址"), 
	
	IPV4("((25[0-5]|2[0-4]\\d|1\\d{2}|[1-9]?\\d)(\\.(25[0-5]|2[0-4]\\d|1\\d{2}|[1-9]?\\d)){3})", "IPV4"),
	
	IPV6(StringUtils.join(
	...
	SOCKET(StringUtils.join("(", IPV4, ":", PORT, ")"), "套接字"),
	
	MD5(StringUtils.join("(([0-9|A-F|a-f]{16}){1,2})"), "MD5码"),
	...
	private RegexEnum(String val, String des) {
		this.VAL = val;
		this.DES = des;
