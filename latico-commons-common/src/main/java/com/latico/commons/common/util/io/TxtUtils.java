...
public class TxtUtils {
	 ...
	 */
	public static String toTXT(String filePath) throws IOException {
	 ...
	 * @return TXT文件路径（若转码失败则返回空串）
	 */
	public static String toTXT(File file) throws IOException {
		String txtPath = "";
		if(file != null) {
			...
			if(!file.getAbsolutePath().equals(srcPath)) {
				FileUtils.deleteQuietly(srcPath);
			}
		}
		return txtPath;
	}
	 ...
	 */
	public static boolean toTXT(String srcPath, String txtPath) throws IOException {
		...
		try {
			...
			while ((rc = fis.read(buff, 0, UnitUtils._1_MB)) > 0) {
				String hex = NumConvertUtils.toHexUpperCase(buff, 0, rc);
				fos.write(hex.getBytes(CharsetType.ISO));
			}
			fos.flush();
			fos.close();
			fis.close();
	 ...
	 */
	public static String toFile(String txtPath) throws IOException {
	 ...
	 * @return 恢复后的文件路径（若恢复失败则返回空串）
	 */
	public static String toFile(File txtFile) throws IOException {
		String snkPath = "";
	 ...
	 */
	public static boolean toFile(String txtPath, String snkPath) throws IOException {
		boolean isOk = false;
		...
		try {
			...
			int rc = 0;
			while ((rc = fis.read(buff, 0, UnitUtils._1_MB)) > 0) {
				String hex = new String(buff, 0, rc, CharsetType.ISO);
				fos.write(NumConvertUtils.toBytes(hex));
			}
			fos.flush();
			fos.close();
			fis.close();
