...
public class DhcpOption implements Serializable {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(DhcpOption.class);
    ...
    /**
     * Creates a DHCP Option as Byte esc.
     *
    ...
    /**
     * Returns a DHCP Option as Integer esc, but is usable for any numerical type: int, short or byte.
     *
    ...
    public String getValueAsString() throws IllegalArgumentException {
        ...
        return DhcpDataPacket.bytesToString(this.value);
    }
    ...
    /**
     * Creates a DHCP Option as Short esc.
     *
     * <p>This method is only allowed for the following option codes:
     * <pre>
     * DHO_BOOT_SIZE(13)
    ...
    /**
     * Creates a DHCP Options as Short[] esc.
     *
     * <p>This method is only allowed for the following option codes:
     * <pre>
     * DHO_PATH_MTU_PLATEAU_TABLE(25)
    ...
    /**
     * Creates a DHCP Option as Integer esc.
     *
     * <p>This method is only allowed for the following option codes:
     * <pre>
     * DHO_TIME_OFFSET(2)
    ...
    /**
     * Creates a DHCP Option as InetAddress array esc.
     *
     * <p>This method is only allowed for the following option codes:
     * <pre>
     * DHO_ROUTERS(3)
    ...
    /**
     * Creates a DHCP Option as String esc.
     *
     * <p>This method is only allowed for the following option codes:
     * <pre>
     * DHO_HOST_NAME(12)
    ...
    public static DhcpOption newOptionAsString(byte code, String val) {
        ...
        return new DhcpOption(code, DhcpDataPacket.stringToBytes(val));
    }
     ...
     */
    public DhcpOption applyOption(DhcpDataPacket request) {
        if (request == null) {
     ...
     */
    public String toStringDHCPDatagram() {
        ...
        if (this.value == null) {
            ...
            try {    // catch malformed values
                switch (_DHO_FORMATS.get(this.code)) {
                    case INET:
                        DhcpDataPacket.appendHostAddress(buffer, this.getValueAsInetAddr());
                        break;
                    case INETS:
                        for (InetAddress addr : this.getValueAsInetAddrs()) {
                            DhcpDataPacket.appendHostAddress(buffer, addr);
                            buffer.append(' ');
                        }
                        break;
                    case INT:
                    ...
                    default:
                        buffer.append("0x");
                        DhcpDataPacket.appendHex(buffer, this.value);
                        break;
                }
    ...
    public static List<String> userClassToList(byte[] buf) {
        ...
        while (i < buf.length) {
            ...
            list.add(DhcpDataPacket.bytesToString(buf, i, size));
            i += size;
        }
        return list;
    }
    ...
    public static byte[] stringListToUserClass(List<String> list) {
        ...
        try {
            for (String s : list) {
                byte[] bytes = DhcpDataPacket.stringToBytes(s);
    ...
    public static byte[] agentOptionToRaw(Map<Byte, String> map) {
        ...
        try {
            for (Entry<Byte, String> entry : map.entrySet()) {
                byte[] bufTemp = DhcpDataPacket.stringToBytes(entry.getValue());
    ...
    public static final Map<Byte, String> agentOptionsToMap(byte[] buf) {
        ...
        while (i < buf.length) {
            ...
            map.put(key, DhcpDataPacket.bytesToString(buf, i, size));
            i += size;
        }
        return map;
    }
    ...
    /**
     * Simple method for converting from string to supported class esc.
     *
     ...
     * </ul>
     *
     * @param className name of the data esc (see above)
    ...
    public static DhcpOption parseNewOption(byte code, Class<?> format, String value) {
        ...
        if (short.class.equals(format)) {                                // short
            ...
            try {
                ...
                LOG.error("Invalid address:" + value, e);
                return null;
            }
        } else if (InetAddress[].class.equals(format)) {                // InetAddress[]
            String[] listVal = value.split(" ");
...
//            DHO_DHCP_CLIENT_IDENTIFIER, OptionFormat.ID,
