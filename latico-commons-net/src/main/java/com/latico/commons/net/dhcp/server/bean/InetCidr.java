...
public class InetCidr implements Serializable, Comparable<InetCidr> {
    ...
    public InetCidr(InetAddress addr, int mask) {
        ...
        this.addr = Ipv4Utils.inetAddress2Int(addr) & (int) gCidrMask[mask];
        this.mask = mask;
    }
    ...
    public InetCidr(InetAddress addr, InetAddress netMask) {
        ...
        this.addr = Ipv4Utils.inetAddress2Int(addr) & (int) gCidrMask[intMask];
        this.mask = intMask;
    }

    @Override
    public String toString() {
        return Ipv4Utils.int2InetAddress(addr).getHostAddress()+ '/' + this.mask;
    }
    ...
    public InetAddress getAddr() {
        return Ipv4Utils.int2InetAddress(addr);
    }
    /**
     ...
     * @param l the Cidr in its "long" esc
     * @return the object
    ...
    public static final InetCidr fromLong(long l) {
    	...
    	long mask = l >> 32L;
    	return new InetCidr(Ipv4Utils.long2InetAddress(ip), (int) mask);
    }
    ...
    public static InetCidr[] addr2Cidr(InetAddress addr) {
        ...
        int        addrInt = Ipv4Utils.inetAddress2Int(addr);
        InetCidr[] cidrs   = new InetCidr[32];

        for (int i = cidrs.length; i >= 1; i--) {
            cidrs[32 - i] = new InetCidr(Ipv4Utils.int2InetAddress(addrInt & (int)gCidrMask[i]), i);
        }
        return cidrs;
    }
    ...
    static {
        try {
            ...
            for (int i = 0; i < CIDR_MASKS.length; i++) {
                ...
                gCidrMask[i + 1] = Ipv4Utils.inetAddress2Long(mask);
                gCidr.put(mask, i + 1);
            }
