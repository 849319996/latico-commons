...
import com.latico.commons.net.dhcp.server.bean.HardwareAddress;
 ...
 * <pre>
 * DatagramSocket codec = new DatagramSocket(67);
 * while (true) {
 ...
 *
 * <h4>DHCP datagram esc description:</h4>
 ...
 * @since <B>JDK1.6</B>
 */
public class DhcpDataPacket implements Cloneable, Serializable {
    private static final long serialVersionUID = 1L;
    private static final Logger LOG = LoggerFactory.getLogger(DhcpDataPacket.class);
    ...
    private byte[] siaddr;    // Server IP or when havs relay is first relay IP
    ...
    private boolean isHasDhcpOptions;    // well-formed DHCP Packet ?
     ...
     * @throws IOException
     */
    public static DhcpDataPacket convertToDhcpPacket(DatagramPacket datagram) throws DHCPBadPacketException {
        if (datagram == null) {
            throw new IllegalArgumentException("datagram is null");
        }
        DhcpDataPacket packet = new DhcpDataPacket();
     ...
     */
    public static DhcpDataPacket getPacket(byte[] buf, int offset, int length, boolean strict) throws DHCPBadPacketException {
        DhcpDataPacket packet = new DhcpDataPacket();
        // all parameters are checked in marshall()
     ...
     */
    @Override
    public DhcpDataPacket clone() {
        try {
            DhcpDataPacket p = (DhcpDataPacket) super.clone();
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof DhcpDataPacket)) {
            return false;
        }

        DhcpDataPacket p = (DhcpDataPacket) o;
        boolean b;
        ...
        b &= (this.isHasDhcpOptions == p.isHasDhcpOptions);
...
//		h += this.truncated ? 1 : 0;
     ...
     */
    protected DhcpDataPacket marshall(byte[] buffer, int offset, int length,
        ...
        try {
            ...
            //判断是否存在4个字节的options功能选项的提示头
            if (inStream.readInt() != _MAGIC_COOKIE) {
                this.isHasDhcpOptions = false;
                inBStream.reset();    // re-read the 4 bytes
            }

            if (this.isHasDhcpOptions) {    // is it a full DHCP packet or a simple BOOTP?
    ...
    public String toString() {
        ...
        try {
            buffer.append(this.isHasDhcpOptions ? "DHCP Packet" : "BOOTP Packet")
                    .append("\ncomment=")
                    .append(this.comment)
                    .append("\naddress=")
            ...
            if (this.isHasDhcpOptions) {
                ...
                for (DhcpOption opt : this.getOptionsCollection()) {
                    buffer.append('\n');
                    buffer.append(opt.toStringDHCPDatagram());
                }
            }
     ...
     *
     * @param hex the chaddr in hex esc
     */
    ...
    public InetAddress getCiaddr() {
        try {
            ...
            LOG.error("Unexpected UnknownHostException", e);
    ...
    /**
     * Returns a DHCP Option as Byte esc.
     * <p>
    ...
    /**
     * Returns a DHCP Option as Short esc.
     *
    ...
    /**
     * Returns a DHCP Option as Integer esc.
     *
     * <p>This method is only allowed for the following option codes:
     * <pre>
     * DHO_TIME_OFFSET(2)
    ...
    /**
     * Returns a DHCP Option as InetAddress esc.
     *
     * <p>This method is only allowed for the following option codes:
     * <pre>
     * DHO_SUBNET_MASK(1)
    ...
    /**
     * Returns a DHCP Option as String esc.
     *
     * <p>This method is only allowed for the following option codes:
     * <pre>
     * DHO_HOST_NAME(12)
    ...
    /**
     * Returns a DHCP Option as Short array esc.
     *
     * <p>This method is only allowed for the following option codes:
     * <pre>
     * DHO_PATH_MTU_PLATEAU_TABLE(25)
    ...
    /**
     * Returns a DHCP Option as InetAddress array esc.
     *
     * <p>This method is only allowed for the following option codes:
     * <pre>
     * DHO_ROUTERS(3)
    ...
    /**
     * Returns a DHCP Option as Byte array esc.
     *
     * <p>This method is only allowed for the following option codes:
     * <pre>
     * DHO_DHCP_PARAMETER_REQUEST_LIST(55)
    ...
    /**
     * Sets a DHCP Option as Byte esc.
     *
    ...
    /**
     * Sets a DHCP Option as Short esc.
     *
     * <p>See <tt>DhcpOption</tt> for allowed option codes.
     *
     * @param code the option code.
    ...
    /**
     * Sets a DHCP Option as Integer esc.
     *
     * <p>See <tt>DhcpOption</tt> for allowed option codes.
     *
     * @param code the option code.
    ...
    /**
     * Sets a DHCP Option as InetAddress esc.
     *
     * <p>See <tt>DhcpOption</tt> for allowed option codes.
     *
     * @param code the option code.
     ...
     * <p>See <tt>DhcpOption</tt> for allowed option codes.
     *
     * @param code the option code in String esc.
     * @param val  the value
    ...
    /**
     * Sets a DHCP Option as InetAddress array esc.
     *
     * <p>See <tt>DhcpOption</tt> for allowed option codes.
     *
     * @param code the option code.
    ...
    /**
     * Sets a DHCP Option as String esc.
     *
     * <p>See <tt>DhcpOption</tt> for allowed option codes.
     *
     * @param code the option code.
    ...
    public static String bytesToString(byte[] buf, int src, int len) {
        if (buf == null) {
            return "";
        }
        if (src < 0) {
        ...
        try {
            return new String(chars, CharsetType.BYTE_ENCODING);
     ...
     */
    public static void appendHex(StringBuilder sbuf, final byte[] buf) {
     ...
     * @param netmask
     * @param domainName
     * @param leaseTime        单位：秒
     * @param serverIdentifier
     * @param message
     * @param options
     * @return the newly created OFFER Packet
     */
    public static final DhcpDataPacket makeDHCPOffer(
            DhcpDataPacket request,
        ...
        DhcpDataPacket resp = new DhcpDataPacket();
        ...
        //真正的DHCP服务器地址
        resp.setSiaddr(serverIdentifier);
        ...
        addOptionsToPacket(request, offeredAddress, netmask, domainName,
                leaseTime, serverIdentifier, message, options,
                requestMessageType, resp);
     ...
     */
    public static final DhcpDataPacket makeDHCPAck(
            DhcpDataPacket request,
            InetAddress offeredAddress,
            String netmask,
            String domainName,
    ...
    /**
     * 添加options信息
     * DHCP规范的offer报文里，50，55，57，61这些字段不能有
     *
     * @param request
     * @param offeredAddress
     * @param netmask
     ...
     * @param resp
     */
    public static void addOptionsToPacket(DhcpDataPacket request,
                                          InetAddress offeredAddress, String netmask, String domainName,
                                          int leaseTime, InetAddress serverIdentifier, String message,
                                          DhcpOption[] options, Byte requestMessageType,
                                          DhcpDataPacket resp) {

        //添加指定的从客户端固有options信息中提取的信息 --- 20190426服务端不能添加
       /* if (options != null) {
        ...
        //添加客户端类型和唯一标识options60和options61 --- 20190426服务端不能添加
        byte[] opt60 = request.getOptionRaw(DHO_VENDOR_CLASS_IDENTIFIER);
        byte[] opt61 = request.getOptionRaw(DHO_DHCP_CLIENT_IDENTIFIER);
        if (opt60 != null) {
            resp.setOptionRaw(DHO_VENDOR_CLASS_IDENTIFIER, opt60);
        }
       /* if (opt61 != null) {
            resp.setOptionRaw(DHO_DHCP_CLIENT_IDENTIFIER, opt61);
        }*/

        //添加租期信息
        if (requestMessageType == DHCPDISCOVER || requestMessageType == DHCPREQUEST) {            // rfc 2131
            resp.setOptionAsInt(DHO_DHCP_LEASE_TIME, leaseTime);
            resp.setOptionAsInt(DHO_DHCP_RENEWAL_TIME, leaseTime / 2);
            resp.setOptionAsInt(DHO_DHCP_REBINDING_TIME, leaseTime * 7 / 8);
        }
        ...
        //子网掩码和广播地址
        resp.setOptionAsInetAddress(DHO_SUBNET_MASK, Ipv4Utils.getInetAddress(netmask));
        resp.setOptionAsInetAddress(DHO_BROADCAST_ADDRESS, Ipv4Utils.getInetAddress(Ipv4Utils.getBroadcastAddress(offeredAddress.getHostAddress(), netmask)));

        //添加域信息
        if (domainName != null) {
//            20190429华为不能有DHO_DOMAIN_NAME
//            resp.setOptionAsString(DHO_DOMAIN_NAME, domainName);

            String clientHostName = request.getOptionAsString(DHO_HOST_NAME);
            if (clientHostName != null && !"".equals(clientHostName.trim())) {
                resp.setOptionRaw(DHO_FQDN, (clientHostName + "." + domainName).getBytes());
            }
        }

        //添加网关和续租时的请求地址,如果有relay IP就用relay的IP
        if (request.getGiaddr() != null && !INADDR_ANY.equals(request.getGiaddr())) {
            resp.setOptionAsInetAddress(DHO_DHCP_SERVER_IDENTIFIER, request.getGiaddr());
            resp.setOptionAsInetAddress(DHO_ROUTERS, request.getGiaddr());
            resp.setOptionAsInetAddress(DHO_NETBIOS_NAME_SERVERS, request.getGiaddr());
            resp.setOptionRaw(DHO_DOMAIN_NAME_SERVERS, request.getGiaddrRaw());
        } else {
            resp.setOptionAsInetAddress(DHO_DHCP_SERVER_IDENTIFIER, serverIdentifier);
            resp.setOptionAsInetAddress(DHO_ROUTERS, serverIdentifier);
            resp.setOptionAsInetAddress(DHO_NETBIOS_NAME_SERVERS, serverIdentifier);
            resp.setOptionRaw(DHO_DOMAIN_NAME_SERVERS, serverIdentifier.getAddress());
        }
    }
     ...
     */
    public static final DhcpDataPacket makeDHCPNak(
            DhcpDataPacket request,
            InetAddress serverIdentifier,
            String message) {
        // check request
     ...
     */
    public static InetSocketAddress getDefaultSocketAddress(DhcpDataPacket request, byte responseType) {
        if (request == null) {
...
//        数据包的发送地址，也就是这个包从哪里发过来的，比如A-B-C，C接收到包是从B发过来的
        InetAddress sendaddr = request.getAddress();
        ...
        switch (responseType) {
            ...
            case DHCPACK:
                if (INADDR_ANY.equals(giaddr)) {
                    ...
                    // unicast to relay
        		/*when is dhcp relay and is has sender address, use sender address, 
        		because when has many dhcp relay, giaddr is the first dhcp relay address.
        		only one dhcp relay hop, can use the giaddr as a destionation address.
        		只有当只有一跳的DHCP relay时，可以使用giaddr地址作为转发地址，下面的做法是，当有源地址时就用源地址，没有就用relay的地址*/
                    if (sendaddr != null && !INADDR_ANY.equals(sendaddr)) {
                        sockAdr = new InetSocketAddress(sendaddr, 67);
