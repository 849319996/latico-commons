...
public class HardwareAddress implements Serializable {
	...
	public HardwareAddress(String macHex) {
		this(DhcpDataPacket.hex2Bytes(macHex));
	}
	
	public HardwareAddress(byte hType, String macHex) {
		this(hType, DhcpDataPacket.hex2Bytes(macHex));
	}
    ...
    public String getHardwareAddressHex() {
    	return DhcpDataPacket.bytes2Hex(this.hardwareAddress);
    }
    
	/**
	 * Prints the hardware address in hex esc, split by ":".
	 */
	@Override
	...
	/**
	 * Parse the MAC address in hex esc, split by ':'.
	 * 
